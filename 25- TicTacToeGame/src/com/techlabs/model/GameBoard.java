...
public class GameBoard {
    ...
    public void placeSymbol(int row, int col, Symbol symbol) {
    	board[row][col] = symbol.name().charAt(0);    //.name is a method defined for all Java enums that returns the constant’s name as a String. 
    }												//Symbol.name would return X or O which will be fetched by charAt(0)
    ...
    public boolean isWinningMove(int row, int col, Symbol symbol) {
        ...
        // Check row = Checks if all cells in the move’s row contain the same symbol (s)
        if (board[row][0] == s && board[row][1] == s && board[row][2] == s) return true;
        // Check column = Checks if all cells in the move’s column contain s
        if (board[0][col] == s && board[1][col] == s && board[2][col] == s) return true;
        // Diagonals
        if (row == col && board[0][0] == s && board[1][1] == s && board[2][2] == s) return true;      //diagonal win (from top-left to bottom-right)
        if (row + col == 2 && board[0][2] == s && board[1][1] == s && board[2][0] == s) return true;    //top-right to bottom-left (anti-diagonal)
        return false;
    }
}
